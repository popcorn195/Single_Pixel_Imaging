...
function [U, metrics] = run_inverse(img_path, phi_path, meas_path, num_meas)
    ...
    tic;
        invA=pinv(A);
        U=invA*y;
        fprintf('Inverse done in %.2f seconds\n', toc);
    ...
    title(sprintf('Inverse Problem Reconstruction\nSSIM: %.4f | PSNR: %.2fdB', ...
          metrics.ssim_val, metrics.psnr_val));
    
    sgtitle(sprintf('Inv Prob | measurements=%d', num_meas));
    
    saveas(gcf, sprintf('results/figures/invp_N%d.png', num_meas));
